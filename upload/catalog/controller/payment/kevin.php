...
*/
use Kevin\Client;
class ControllerPaymentKevin extends Controller {
	...
	//	date_default_timezone_set('Europe/Vilnius');	
		require_once dirname(dirname(__DIR__)) . '/model/payment/kevin/vendor/autoload.php';
        ...
        $this->load->model('payment/kevin');
        $this->load->language('payment/kevin');
		...
		$data['kevin_instr_title'] = $this->config->get('kevin_ititle' . $current_language);
	//	$data['kevin_instr'] =  html_entity_decode($this->config->get('kevin_instruction' . $current_language));
		$data['kevin_instr'] =  $this->config->get('kevin_instruction' . $current_language);
		...
		if (file_exists(DIR_APPLICATION . 'controller/payment/kevin_image/credit_card_icon.png')) {
            $data['credit_card_icon'] = $this->config->get('config_url') . 'catalog/controller/payment/kevin_image/credit_card_icon.png';
        } else {
            $data['credit_card_icon'] = '';
        }
	//	$data['credit_card_icon'] = $this->config->get('credit_card_icon');
		
		$data['payment_methods'] = $payment_methods['data'];
		$data['banks'] = $banks['data'];
		//echo '<pre>';	print_r($data['banks']); echo '</pre>';
		$data['action'] = $this->url->link('payment/kevin/redirect');
		$data['bank_name_enable'] = $this->config->get('kevin_bank_name_enabled');
		...
		} else if (file_exists(DIR_TEMPLATE . 'default/template/' . $this->type . '/' . $this->name . '.tpl')) {
	...
	//	date_default_timezone_set('Europe/Vilnius');
		...
		$confirm_url = $this->url->link('payment/kevin/confirm');
        $webhook_url = $this->url->link('payment/kevin/webhook');
		...
		if ($payment_method == 'bank') {
			$payment_attr = [
				...
				'bankPaymentMethod'       => [
					'endToEndId'          	=> strval($order_id),
					'creditorName'          => $this->config->get('kevin_client_company'),
					'creditorAccount'       => [
						'iban'              	=> $this->config->get('kevin_client_iban')
					],
				],
			];
		...
		$this->model_payment_kevin->addKevinOrder($order_info, $init_payment, $ip_address, $order_status_id, $total, $payment_method, $bank_id);
    ...
    public function confirm() {
        ...
        $this->language->load('payment/kevin');
        $this->load->model('checkout/order');
        $this->load->model('payment/kevin');
		...
		if ($old_status_id != $new_status_id && $order_info['order_id'] == $this->session->data['order_id']) {
			$this->KevinLog($log_data);
			$this->model_payment_kevin->updateConfirmKevinOrder($payment_id, $get_payment_status, $new_status_id, $payment_method);
			$payment_status = true;
		} else {
    ...
    public function webhook() {
		...
		*/
		...
		if ($old_status_id != $new_status_id && $order_info['order_id'] == $order_id) {
			$this->KevinLog($log_data);
			$this->model_payment_kevin->updateWebhookKevinOrder($payment_id, $get_payment_status, $new_status_id, $payment_method);
			$payment_status = true;
		} else {
			$payment_status = false;
		}
